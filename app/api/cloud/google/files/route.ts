...
function sortGoogleFiles<T extends DriveFile>(files: T[]) {
...
export async function GET(req: NextRequest) {
  try {
    ...
    const accounts = await listCloudAccounts(access.user.id, "google");
    ...
    if (!requestedAccountId && folderId === "root" && accounts.length > 1) {
      const providerFiles = await Promise.all(
        accounts.map(async (account) => {
          ...
          const files = await listGoogleDriveFiles(accessToken, "root", query);
          return sortGoogleFiles(files).map((file) => toResponseFile(file, account));
        })
      );
      ...
      return NextResponse.json({
        files: providerFiles
          ...
          .sort((left, right) => {
            const leftIsFolder =
              left.mimeType === "application/vnd.google-apps.folder";
            const rightIsFolder =
              right.mimeType === "application/vnd.google-apps.folder";
            if (leftIsFolder !== rightIsFolder) {
              return leftIsFolder ? -1 : 1;
            }
            return left.name.localeCompare(right.name);
    ...
    if (!account) {
      return NextResponse.json(
        { error: "Selected Google Drive account was not found" },
        { status: 400 }
      );
    }

    const accessToken = await getCloudAccessToken(account);
    const files = await listGoogleDriveFiles(accessToken, remoteFolderId, query);
    return NextResponse.json({
      files: sortGoogleFiles(files).map((file) => toResponseFile(file, account)),
    });
  } catch (error) {
    if (
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const accessToken = await getCloudAccessToken(account);
    await deleteGoogleDriveFile(accessToken, remoteFileId);
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (action === "move") {
      ensureScopedAccountMatch(sourceParentId, account.id, "Source folder");
      ensureScopedAccountMatch(targetParentId, account.id, "Target folder");
      const file = await moveGoogleDriveFile(
        accessToken,
        remoteFileId,
        getScopedCloudRemoteId(sourceParentId) || "root",
    ...
    const file = await renameGoogleDriveFile(accessToken, remoteFileId, normalizedName);
    return NextResponse.json({ file: toResponseFile(file, account) });
  } catch (error) {
    if (
      error instanceof Error &&
